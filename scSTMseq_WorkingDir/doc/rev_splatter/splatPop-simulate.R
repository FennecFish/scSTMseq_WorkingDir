...
splatPopSimulateSC <- function(sim.means,
    ...
    withr::with_seed(seed, {
        ...
        # revise
        if (length(group.prob[[1]]) != length(sim.means)) {
            group.prob[[1]] <- rep(1 / length(sim.means), length(sim.means))
        }
        
        if (length(group.prob[[2]]) != length(sim.means)) {
            group.prob[[2]] <- rep(1 / length(sim.means), length(sim.means))
        }
        ...
        if (is.null(conditions)) {
            conditions <- splatPopDesignConditions(params, samples, batches)
        }
        ...
        #         ceiling(group.prob[[i]][which(group.names == group)] * batchCells[[i]])
        #     })
        # }), group.names)
        ...
        for (g in group.names) {
            ...
            sims <- lapply(samples, function(x) {
                #cat(x, "with Group Number ",g, "is", group.n[[x]][g][[1]], "\n")
                paramsG <- setParams(params, batchCells = unlist(group.n[[x]][g]))
    ...
    for (b in batch.list) {
        ...
        if (method != "single") {
            if(is.null(t)){
                ...
                groups <- sample(
                    seq_len(nGroups), nCells,
                    prob = group.prob[[t]], replace = TRUE
                )
                colData(sim)$Group <- factor(
                    group.names[groups],
                    levels = group.names
...
#'
#' @keywords internal
splatPopDesignConditions <- function(params, samples, batches) {
    condition.prob <- getParam(params, "condition.prob")
    ...
    if (length(condition.prob) == 1) {
        ...
        # # revise here! 
        # # assign condition to each cells within a sample, instead assign to samples
        # nBatches <- getParam(params, "nBatches")
        # batch.cells <- getParam(params, "batchCells")
        # batch.size <- getParam(params, "batch.size")
        # # total number of cells for each sample
        # total_cell <- sum(batch.cells * batch.size)
        # ll <- ceiling(condition.prob * total_cell)
        # conditions <- unlist(lapply(seq_along(ll), function(x) {
        #     rep(paste0("Condition", x), ll[[x]])
        # }))
        # conditions <- sample(conditions, total_cell)
