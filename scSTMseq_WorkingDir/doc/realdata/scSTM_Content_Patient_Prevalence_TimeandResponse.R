...
#                                   prevalence = ~timepoint + patient_id, content = ~timepoint,
#                                   max.em.its = 10, init.type = "TopicScore", seed = 100)
scSTM.mod <- selectModel_parallel(sce = dat, sample = "patient_id", K = ngroup,
                          prevalence = ~timepoint + expansion, content = ~patient_id,
...
saveRDS(scSTM.mod, file = "/work/users/e/u/euphyw/scLDAseq/data/PD1_data/PD1_scSTM/scSTM_Content_Time_Prevalence_TimeandSample.rds")
