...
manova.dir <- "Manova_Seurat"
paths <- Sys.glob(file.path(dir, "*nCellType10_Batch_*", manova.dir)) 
files <- unlist(lapply(paths, function(path) list.files(path, pattern = "Manova_seurat_pValue", full.names = TRUE)))

res <- vector(mode = "list")

for(file_name in files){
  set_level <- sub("^Manova_seurat_pValue_(.*)\\.rds$", "\\1", basename(file_name))
...
saveRDS(res, file = paste0("res/1MultiSample_SingleResponse_Simulation/Manova_Seurat_nSample20_nCellType10_Batch_CancerCell.rds"))
