...
thetapost.global <- function(model, nsims) {
  ...
  # psi <- model$psi$alpha
  # 
  # repetition_counts <- table(model$sampleID)
  # # subtract to get the topic variaiton coming from only metadata
  # expanded_psi <- do.call(rbind, lapply(1:nrow(psi), function(i) {
