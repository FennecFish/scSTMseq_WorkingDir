...
get_transformed_data <- function(seurat.sims, transform="logit"){
  ...
  time <- seurat.sims$Time
  tab <- table(sample, clusters, time)
  ...
  new_sample <- rownames(prop.trans)
  prop.trans <- rbind(prop.trans[, , 1], prop.trans[, , 2])
  colnames(prop.trans) <- paste0("Cluster", 1:ncol(prop.trans))
  prop.trans <- t(prop.trans)
  meta.prop <- data.frame(
    Sample = rep(new_sample,2),
    Time = rep(c(1,2), each = length(new_sample))
  )
  
  response_table <- as.data.frame(as.table(table(seurat.sims$Response, sample))) %>% filter(Freq > 0)
  
  meta.prop$Response <- response_table$Var1[match(meta.prop$Sample, response_table$sample)]
  return(list(prop.trans = prop.trans, meta.prop = meta.prop))
}

update_propeller <- function(prop.trans, trend=FALSE, robust=TRUE){
  meta.prop <- prop.trans$meta.prop
  prop.trans <- prop.trans$prop.trans
  sample <- meta.prop$Sample
  group <- meta.prop$Time
  response <- meta.prop$Response
  
  cov <- factor(paste(response,group,sep="."))
  design<-model.matrix(~ 0 + cov)
  colnames(design) <- c("nonR_t1", "nonR_t2", "R_t1", "R_t2")
  
  fit<-lmFit(prop.trans,design)
  contrasts <- makeContrasts(Response_Change = R_t2 - R_t1,
                             nonResponse_Change = nonR_t2 - nonR_t1,
                             levels = design)
...
saveRDS(ARI, paste0("res/1MultiSample_MultiResponse_Simulation/seurat_ARI/ARI_",basename(dir), "_", set_level, ".rds"))
...
seurat.sims$Response <- colData(sims)$Response[match(rownames(seurat.sims@meta.data),colData(sims)$Cell)]

prop.trans.logit <- get_transformed_data(seurat.sims = seurat.sims, transform="logit")
out.logit <- update_propeller(prop.trans.logit)

dir_path = paste0(dir, "/propeller_unpaired/")
