...
res <- read.csv("res/scSTM_l3_adjRandIndex_t4.csv")

#change format from wide to long
res_long <- res %>% 
  gather(method, adjR, scSTM_select_cluster:stm_cluster, factor_key=TRUE) %>%
  drop_na()
...
res_stats <- res_long %>%
  ...
  summarize(mean_adjR = mean(adjR, na.rm = TRUE),
            ...
            .groups = 'drop') %>% 
  separate(level, into = c("control", "l"), sep = "_", remove = FALSE)

#### compare all methods ####
all_res_stats <- res_stats %>%
  mutate(line_type = ifelse(grepl("^scSTM", method), "dashed", "solid")) 

png("res/figure1_mean_adj_single_sample.png", width = 2500, height = 1500, res = 300)
ggplot(all_res_stats, aes(x = l, y = mean_adjR, color = method, group = method, linetype = line_type)) +
